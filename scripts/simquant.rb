			...
			File.open(@output[:xhtmlReport], 'w') do |lk_Out|
				...
				lk_Peptides.each do |ls_Peptide|
					...
					lk_PeptideResults.each do |lk_Row|
						...
						lk_Out.puts "<tr><td>#{lk_Row['file']}</td><td>#{lk_Row['charge']}</td><td>#{sprintf("%1.2f", lk_Row['ratio'].to_f)}</td><td>#{sprintf("%1.2f", lk_Row['certainty'].to_f * 100.0)} %</td><td><span class='toggle' onclick=\"toggle('#{ls_Name}')\">#{lk_SubRows.size} scans</span></td></tr>"
						lk_SubRows.each do |lk_SubRow|
							ls_Svg = File::read(File::join(ls_SvgPath, lk_SubRow['svg'] + '.svg'))
							ls_Svg.sub!(/<\?xml.+\?>/, '')
							ls_Svg.sub!(/<svg width=\".+\" height=\".+\"/, "<svg ")
							lk_Out.puts "<tr class='sub #{ls_Name}' style='display: none;'><td>#{lk_Row['file']}</td><td>#{lk_Row['charge']}</td><td>#{sprintf("%1.2f", lk_SubRow['ratio'].to_f)}</td><td>#{sprintf("%1.2f", lk_SubRow['certainty'].to_f * 100.0)} %</td><td></td></tr>"
							lk_Out.puts "<tr class='sub #{ls_Name}' style='display: none;'><td colspan='5'>"
							lk_Out.puts "<div>#{lk_Row['file']} ##{lk_SubRow['id']} @ #{sprintf("%1.2f", lk_SubRow['retentionTime'].to_f)} minutes</div>"
							lk_Out.puts ls_Svg
							lk_Out.puts "</td></tr>"
						end
					end
